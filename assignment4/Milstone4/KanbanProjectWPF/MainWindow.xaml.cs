...
namespace KanbanProjectWPF
{
    ...
    public partial class MainWindow : INotifyPropertyChanged
    {
        ...
        private ObservableCollection<BoardsNamePresention> boards;
        private Hashtable boardsCollection;
        ...
        public void addMenuItem(String boardName)
        {
            MenuItem newExistMenuItem = (MenuItem)this.boardsNameMenu.Items[0];
            MenuItem newExistMenuItemForDelelte = (MenuItem)this.boardsDeleteNameMenu.Items[0];
            newExistMenuItem.Items.Clear();
            newExistMenuItemForDelelte.Items.Clear();
            foreach (BoardsNamePresention s in this.boards)
            {
                MenuItem mi = new MenuItem();
                MenuItem miForDelete = new MenuItem();

                mi.Header = s.BoardName;
                miForDelete.Header = s.BoardName;

                if (boardName == s.BoardName)
                {
                    this.currBoard = boardName;
                    mi.Background = new SolidColorBrush(Colors.IndianRed);
                    mi.ToolTip = "Selected board";
                } else
                {
                    mi.Background = new SolidColorBrush(Colors.Gainsboro);
                    miForDelete.Background = new SolidColorBrush(Colors.Gainsboro);
                    miForDelete.ToolTip = "Click To delete";

                    newExistMenuItemForDelelte.Items.Add(miForDelete);
                }        
                newExistMenuItem.Items.Add(mi);
            }
        }
        ...
        private void BoardsName_Click(object sender, RoutedEventArgs e)
        {
            this.menuItem = (MenuItem)e.OriginalSource;
            if (this.menuItem != null)
            {
                string selectedBoard = this.menuItem.Header.ToString();
                this.menuItem.Background = new SolidColorBrush(Colors.IndianRed);
                show((Board)this.boardsCollection[selectedBoard]);
                this.MainB = (Board)this.boardsCollection[selectedBoard];
                InfoObject info = SystemInterface.setCurrBaord(this.userName, selectedBoard);
                if (!info.getIsSucceeded())
                {
                    MessageBox.Show(info.getMessage());
                }
        ...
        public void setBaordNames()
        {
            this.boards.Clear();
            foreach (var table in this.boardsCollection.Values)
            {
                this.boards.Add(new BoardsNamePresention(((Board)table).getBoardName()));
            }
        }
        ...
        {
            boardAdd bA = new boardAdd(userName, this);
            bA.Show();
        }
    }
}